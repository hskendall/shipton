...
package flame.crypto
{
	...
	public final class ECDSA extends AsymmetricAlgorithm
	{
		 ...
		 */
		public function signHash(hash:ByteArray):Array
		{

			
			var buffer:ByteArray = new ByteArray();
			
			buffer.writeBytes(hash, 0, Math.min(hash.length, int((_domainParameters.n.bitLength + 7) / 8)));
			
			var e:BigInteger = new BigInteger(buffer);
			var k:BigInteger = new BigInteger(RandomNumberGenerator.getNonZeroBytes(_domainParameters.n.bitLength), true)
			...
			var s:BigInteger = k.modInverse(_domainParameters.n).multiply(e.add(_d.multiply(r))).mod(_domainParameters.n);
			var keySizeInBytes:int = (_keySize + 7) / 8;
			
			buffer.clear();
			buffer.writeBytes(CryptoUtil.ensureLength(r.toByteArray(), keySizeInBytes));
			
			trace("oyeaaaah: "+ String(Hex.fromArray(r.toByteArray())))
			trace("oyeaaaah2: "+ Hex.fromArray(s.toByteArray()))
			...
			returnArray.push(String(Hex.fromArray(r.toByteArray())))
			returnArray.push(String(Hex.fromArray(s.toByteArray())))
		...
		public function verifyHash(hash:ByteArray, signature:ByteArray):Boolean
		{
			...
			var inverse:BigInteger = new BigInteger(buffer, true).modInverse(_domainParameters.n);
			...
			for (var i:int = Math.max(u1.bitLength, u2.bitLength) - 1; i >= 0; i--)
			{
				result = result.double();
				
				if (u1.isBitSet(i))
					result = result.add(u2.isBitSet(i) ? gq : g);
				else if (u2.isBitSet(i))
					result = result.add(q);
			}
		...
		public function modulo(data:ByteArray,data2:ByteArray):BigInteger
		{
			_domainParameters = ECDomainParametersCache.getParametersByKeySize(_keySize);
			_curve = new PrimeEllipticCurve(_domainParameters.q, _domainParameters.a, _domainParameters.b);
			
			//_d = new BigInteger(data).mod(_domainParameters.n.subtract(BigInteger.ONE)).add(BigInteger.ONE);
			var zo2 = new BigInteger(data, true);
			var zo1 = new BigInteger(data2, true);
			
			var sis:BigInteger = BigInteger(zo1).add(BigInteger(zo2)).mod(_domainParameters.n)
			
			trace("ooooooooooooooooooooosisisss: "+ sis)
		...
		private function generateKeyPair():void
		{
            ...
            randomNumb= new BigInteger(RandomNumberGenerator.getNonZeroBytes(_domainParameters.n.bitLength)	)	
			
			
			_d = BigInteger(randomNumb).mod(_domainParameters.n.subtract(BigInteger.ONE)).add(BigInteger.ONE);
		...
		public function deriveCorrespondingPubkey(dataRandomNumb:BigInteger):ECCParameters
		{
			_domainParameters = ECDomainParametersCache.getParametersByKeySize(_keySize);
			_curve = new PrimeEllipticCurve(_domainParameters.q, _domainParameters.a, _domainParameters.b);
			
			_d = BigInteger(dataRandomNumb).mod(_domainParameters.n.subtract(BigInteger.ONE)).add(BigInteger.ONE);
			...
			parameters.y = CryptoUtil.ensureLength(_y.toByteArray(), keySizeInBytes);
									...
									trace("derivedis: "+   Hex.fromArray(parameters.x) )
			...
			public function deriveCorrespondingPubkey2(data:ByteArray):ECCParameters
		{
			_domainParameters = ECDomainParametersCache.getParametersByKeySize(_keySize);
			_curve = new PrimeEllipticCurve(_domainParameters.q, _domainParameters.a, _domainParameters.b);
			
			
			_d = new BigInteger(data, true);
			//_d = BigInteger(data);
